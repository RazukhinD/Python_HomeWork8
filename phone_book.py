...
def change_contact(id):
    ...
    if confirm.lower()=='y':
        ...
        temp=int(input('Введите цифру столбца который хотите изменить: 1. Имя, 2. Телефон, 3. Комментарий: '))
        if temp == 1:
            ...
            comment=input('Введите новое коммент: ')
...
def find_contact(search_text,phone_book):
    ...
    for i in range(len(new_list_with_name)):
        if new_list_with_name[i].find(search_text.lower()):
            return print(f'Вот данные пользователя с таким именем: {phone_book[i]}')
        else:
